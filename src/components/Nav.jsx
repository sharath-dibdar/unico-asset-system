...
export function Sidebar({ view, setView, assets, vendors, audits, onAdd, onImport, onExport, onPrintLabels }) {
  ...
  const navItem = (id, icon, label, badge) => (
    <button key={id} onClick={()=>setView(id)} style={{
      ...
      fontFamily:"'DM Sans',sans-serif", textAlign:"left", width:"100%", transition:"all 0.15s",
    }}>
  ...
  return (
    <div className="dsk" style={{ width:230, background:C.sf, borderRight:`1px solid ${C.br}`, display:"flex", flexDirection:"column", padding:"0 0 16px", flexShrink:0, userSelect:"none", overflowY:"auto" }}>
      ...
      <div style={{ padding:"0 16px 12px" }}>
        <button onClick={onAdd} style={{ width:"100%", background:`${C.ac}18`, border:`1px solid ${C.ac}40`, color:C.ac, cursor:"pointer", padding:"10px", borderRadius:10, fontSize:13, fontWeight:700, fontFamily:"'DM Sans',sans-serif" }}>
          + New Asset
        </button>
      </div>
      ...
      {Object.entries(CATS).map(([k,v]) => {
        ...
        return (
          <button key={k} onClick={()=>setView("list")} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"6px 22px", background:"transparent", border:"none", cursor:"pointer", color:C.mu, fontSize:12, fontFamily:"'DM Sans',sans-serif", width:"100%", transition:"color 0.15s" }}
      ...
      <div style={{ padding:"12px 16px 0", display:"flex", flexDirection:"column", gap:6 }}>
        <button onClick={onImport} style={{ background:C.el, border:`1px solid ${C.br}`, color:C.mu, cursor:"pointer", padding:"8px 12px", borderRadius:8, fontSize:12, fontFamily:"'DM Sans',sans-serif", textAlign:"left" }}>⬆  Import Excel</button>
        <button onClick={onExport} style={{ background:C.el, border:`1px solid ${C.br}`, color:C.mu, cursor:"pointer", padding:"8px 12px", borderRadius:8, fontSize:12, fontFamily:"'DM Sans',sans-serif", textAlign:"left" }}>⬇  Export / Reports</button>
        <button onClick={onPrintLabels} style={{ background:C.el, border:`1px solid ${C.br}`, color:C.mu, cursor:"pointer", padding:"8px 12px", borderRadius:8, fontSize:12, fontFamily:"'DM Sans',sans-serif", textAlign:"left" }}>🖨  Print QR Labels</button>
      </div>

      <div style={{ padding:"14px 22px 0", borderTop:`1px solid ${C.br}`, marginTop:12 }}>
        ...
        {vendors?.length>0 && <div style={{ fontSize:11, color:C.mu2, marginTop:4 }}>{vendors.length} vendor{vendors.length!==1?"s":""} · {(audits||[]).length} audit{(audits||[]).length!==1?"s":""}</div>}
      </div>
    </div>
  );
}
...
export function TopBar({ view, onAdd, onBack, selectedName }) {
  const titles = { dashboard:"Dashboard", list:"All Assets", detail:"Asset Detail", vendors:"Vendor Book", audits:"Physical Audits", audit:"Audit Detail", reports:"Reports", checkout:"Check-In / Out" };
  return (
    <div style={{ background:C.sf, borderBottom:`1px solid ${C.br}`, padding:"14px 22px", display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0, gap:12 }}>
      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
        ...
        <div>
          <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:17, color:C.tx, lineHeight:1 }}>
...
export function BottomNav({ view, setView, onAdd }) {
  const items = [
    ...
    { id:"vendors",   icon:"🏪",l:"Vendors" },
    { id:"audits",    icon:"📋",l:"Audits" },
  ];
  return (
    <div className="mob" style={{ background:C.sf, borderTop:`1px solid ${C.br}`, display:"flex", flexShrink:0, zIndex:10 }}>
      {items.map(n=>(
        <button key={n.id} onClick={()=>setView(n.id)} style={{ flex:1, background:"none", border:"none", cursor:"pointer", color:view===n.id?C.ac:C.mu, padding:"10px 0 8px", display:"flex", flexDirection:"column", alignItems:"center", gap:2, fontSize:16 }}>
          ...
          <span style={{ fontSize:9, fontFamily:"'DM Sans',sans-serif" }}>{n.l}</span>
        </button>
      ))}
      <button onClick={onAdd} style={{ flex:1, background:"none", border:"none", cursor:"pointer", color:C.ac, padding:"10px 0 8px", display:"flex", flexDirection:"column", alignItems:"center", gap:2 }}>
        ...
        <span style={{ fontSize:9, fontFamily:"'DM Sans',sans-serif" }}>Add</span>
      </button>
    </div>
  );
}
