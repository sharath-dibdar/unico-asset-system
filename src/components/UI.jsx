...
export const FieldVal = ({ label, val, mono, accent, wide }) => val ? (
  <div style={wide?{ gridColumn:"1/-1" }:{}}>
    <Label>{label}</Label>
    <div style={{ fontSize:14, fontWeight:500, fontFamily:mono?"'DM Mono',monospace":"inherit", color:accent?C.ac2:C.tx, wordBreak:"break-word" }}>{val}</div>
  </div>
) : null;
...
export const Btn = ({ children, onClick, variant="secondary", disabled, style:s }) => {
  const base = { cursor:disabled?"not-allowed":"pointer", padding:"9px 18px", borderRadius:9, fontSize:13, fontWeight:700, fontFamily:"'DM Sans',sans-serif", border:"none", transition:"opacity 0.15s", opacity:disabled?0.5:1, ...s };
  const v = {
    primary:  { background:C.ac,  color:"#fff" },
...
export const NFCButton = ({ assetCode }) => {
  ...
  return (
    <div style={{ marginTop:8 }}>
      <button onClick={write} disabled={status==="writing"||status==="success"} style={{ width:"100%", background:C.el, border:`1px solid ${cols[status]}40`, color:cols[status], cursor:status==="writing"?"wait":"pointer", padding:"8px 12px", borderRadius:8, fontSize:12, fontWeight:600, fontFamily:"'DM Sans',sans-serif" }}>
        📡 {labs[status]}
      </button>
